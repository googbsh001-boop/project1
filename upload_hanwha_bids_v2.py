            ...
            if file.endswith('.xlsb') and not file.startswith('~'):
                ...
                try:
                    ...
                    try:
                        ...
                        # 예가대비 (낙찰율)
                        idx_est_ratio = -1
                        if '예가대비' in header_row:
                            idx_est_ratio = header_row.index('예가대비')
                        elif '낙찰율' in header_row:
                            idx_est_ratio = header_row.index('낙찰율')
                    ...
                    for _, row in data_df.iterrows():
                        ...
                        if "한화" in company:
                            ...
                            if br != "": base_ratio_val = br
                            
                            er = row[idx_est_ratio] if idx_est_ratio != -1 and not pd.isna(row[idx_est_ratio]) else ""
                            if er != "": est_ratio_val = er
                            
                            pr = row[idx_priority] if idx_priority != -1 and not pd.isna(row[idx_priority]) else ""
                            if pr != "": priority_val = pr
                            
                            break
                            
                    result.append([file, participated, base_ratio_val, est_ratio_val, priority_val])
...
def main():
    ...
    # 데이터 정리 (헤더 변경)
    headers = [["공고명", "입찰유무(한화)", "기초대비 투찰율(%)", "예가대비 낙찰율(%)", "낙찰우선순위"]]
